...
# A snapshot of the firmware already AT its user interface, saved by
# tools/introboot.py. A ladder rung is a point inside the boot intro, so
# opening one means playing the rest of the intro and then waiting for the
# +Drive initialisation before anything is usable; opening this one is a
# restore and the UI is on screen in the first frame. It is preferred
# automatically when present and is purely an optimisation -- delete it and
# everything still works from the ladder.
UI_SNAPSHOT = 'gui.snap'
# Digitakt (mk1) needs a longer ladder than Digitakt II. Its intro is a
# 180-frame animation paced by PIT3 (display task loop at 0x4006ca00..
# 0x4006cb94) and does not hand over to the application task until
# roughly 560M instructions from reset -- measured, not guessed. A
# ladder topping out at 400M therefore resumes INSIDE the intro every
# time, which looks exactly like a deadlocked boot.
# Extending this for Digitakt (mk1) does NOT help and was reverted:
# measured, the cold-boot path produces byte-identical snapshots from
# 280M all the way to 900M (same 50,858 addrs, same 10 tasks, same
# pc=0x4006932e). The firmware does not advance during the cold boot
# because that path does not pace PIT3; the 180-frame intro only runs
# once a snapshot is RESUMED with a Timers armed. So the extra rungs
# cost ~10 minutes and record the same state eight times. What mk1
# actually needs is ~700M instructions of RUN TIME after resume.
...
def paths_for(syx):
    ...
    if config.is_tested(syx):
        snap, prefix = config.snapshot(SNAPSHOT), config.snapshot('boot')
    else:
        stem = os.path.splitext(os.path.basename(syx))[0]
        snap = config.snapshot(os.path.join(stem, SNAPSHOT))
        prefix = config.snapshot(os.path.join(stem, 'boot'))
    ui = os.path.join(os.path.dirname(snap), UI_SNAPSHOT)
