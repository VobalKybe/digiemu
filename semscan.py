...
# The Digitakt II addresses, kept only as the fallback for a caller that has
# no resolved profile. They are NOT build-invariant: the mk1 RTOS is the same
# code shifted +0x2e4, so give/give_b sit at 0x40001770/0x400017e0 there and
# scanning for the DT2 addresses finds nothing at all -- a silent empty
# never-fake set, which looks exactly like "this build has no task posters".
# Pass the profile's resolved `give`/`give_b` instead; emu/symbols.py has Sig
# rules for both that match every build tested.
GIVE = 0x4000148c
GIVE_B = 0x400014fc
VBR = 0x40000000
VBR_END = VBR + 0x400
...
def never_fake_semaphores(image, load_addr=symbols.LOAD_ADDR, modeled_vectors=frozenset(),
    ...
    `give`/`give_b` are the two post primitives' addresses in THIS image; pass
    the resolved profile's. They default to the Digitakt II ones, which are
    wrong for any build whose RTOS sits at a different offset and which then
    silently return an empty set (see the constants' comment).
    ...
    give_b = GIVE_B if give_b is None else give_b
    h = hashlib.sha256(image).hexdigest()
    key = (h, load_addr, frozenset(modeled_vectors), give, give_b)
